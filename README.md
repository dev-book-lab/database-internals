...
| [02. Read Replica — 읽기 분산과 정합성 트레이드오프](./replication-high-availability/02-read-replica-consistency.md) | Replication Lag이 발생하는 이유와 측정 방법(`Seconds_Behind_Master`), 방금 쓴 데이터를 Read Replica에서 읽었을 때 보이지 않는 "Dirty Read" 문제, 쓰기 직후 읽기는 Primary로, 이후 읽기는 Replica로 라우팅하는 전략 |
